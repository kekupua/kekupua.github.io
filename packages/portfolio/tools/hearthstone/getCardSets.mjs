...
const getCards = async () => {
  ...
  fs.writeFileSync(
    "./src/lib/hearthstone-set-metadata.json",
    JSON.stringify(sets, null, 2),
    "utf-8",
  );
  fs.writeFileSync(
    "./src/lib/hearthstone-set-group-metadata.json",
    JSON.stringify(setGroups, null, 2),
    "utf-8",
  );
  sets.forEach(async (set) => {
    const { data: cards } = await client.cardSearch({
      collectible: 1,
      set: set.slug,
    });
    fs.writeFileSync(
      `./public/hearthstone/${set.slug}.json`,
